...
public class MsgsPresenter : MonoBehaviour
{
    ...
    private Dictionary<Msg, MsgView> msgViewsInChanelToServer = new Dictionary<Msg,MsgView>();
    private Dictionary<Msg, MsgView> msgViewsInChanelToClient = new Dictionary<Msg,MsgView>();
    private Dictionary<Msg, MsgView> msgViewsOnServer = new Dictionary<Msg,MsgView>();
    ...
    private void Update()
    {
        ...
        foreach (var msgView in msgViewsInChanelToClient.Values)
        {
            ...
            var m = msgView.Msg;
            msgView.transform.position =
                Vector3.Lerp(server.transform.position, client.transform.position, (Time.time - m.TimestampInChannel) / m.TravelTime) + new Vector3(0, y, 0);
